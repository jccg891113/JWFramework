...
namespace JWFramework
{
	...
	///   JWTouch3DCamera.currentTouchID tells you which button was clicked.
	...
	public class JWTouch3DCamera : MonoBehaviour
	{
		...
		public class MouseOrTouch
		{
			...
			/// </summary>
			public float deltaTime { get { return Time.unscaledTime - pressTime; } }
		}

		/// <summary>
		/// List of all active cameras in the scene.
		/// </summary>
		static public JWList<JWTouch3DCamera> list = new JWList<JWTouch3DCamera> ();
		...
		/// Use JWTouch3DCamera.current to get the current camera.
		/// </summary>
		...
		/// <summary>
		/// JWTouch3DCamera that sent out the event.
		/// </summary>
		static public JWTouch3DCamera current = null;
		...
		static public GameObject selectedObject {
			...
			set {
				...
				if (mCurrentSelection != null) {
					if (shouldRestore) {
						JWTouch3DCamera cam = (mCurrentSelection != null) ? FindCameraForLayer (mCurrentSelection.layer) : JWTouch3DCamera.list [0];
					...
					inputHasFocus = false;//Fix//(mCurrentSelection.activeInHierarchy && mCurrentSelection.GetComponent<UIInput> () != null);
					if (onSelect != null)
		...
		static public Camera mainCamera {
			get {
				JWTouch3DCamera mouse = eventHandler;
		...
		static public JWTouch3DCamera eventHandler {
			get {
				for (int i = 0; i < list.Count; ++i) {
					// Invalid or inactive entry -- keep going
					JWTouch3DCamera cam = list [i];
					if (cam == null || !cam.enabled || !(cam.gameObject && cam.gameObject.activeInHierarchy))
						continue;
					return cam;
				}
				return null;
		...
		static int CompareFunc (JWTouch3DCamera a, JWTouch3DCamera b)
		{
		...
		static JWList<DepthEntry> mHits = new JWList<DepthEntry> ();
		...
		static public bool Raycast (Vector3 inPos)
		{
			for (int i = 0; i < list.Count; ++i) {
				...
				if (!cam.enabled || !(cam.gameObject && cam.gameObject.activeInHierarchy))
					continue;
...
//			}
		...
		static public JWTouch3DCamera FindCameraForLayer (int layer)
		{
		...
		static public MouseOrTouch GetTouch (int id)
		{
			...
			touch.pressTime = Time.unscaledTime;
...
//			NGUIDebug.debugRaycast = debug;
		...
		void Update ()
		{
			...
			if (useMouse && mHover != null) {
				...
				if (showTooltips && mTooltipTime != 0f && (mTooltipTime < Time.unscaledTime ||
		...
		public void ProcessMouse ()
		{
			...
			if (isPressed || posChanged || mNextRaycast < Time.unscaledTime) {
				mNextRaycast = Time.unscaledTime + 0.02f;
			...
			for (int i = 0; i < 3; ++i) {
				...
				if (pressed) {
					...
					currentTouch.pressTime = Time.unscaledTime;
		...
		public void ProcessTouches ()
		{
			...
			for (int i = 0; i < count; ++i) {
				...
				// Double-tap support
				if (tapCount > 1)
					currentTouch.clickTime = Time.unscaledTime;
